FaceBook Album Download Application 

PART 1 :

Login using Facebook credentials.
Ask user to give permission to access of email,cover_photo,name and photos.
Application will fetche all Albums which is added by user or in which user is tagged.
...
A "Move" link(in purple) is displayed for each album.
...
When user clicks on "Move Selected Albums" link, jquery(Ajax) processes PHP script to collect photos for all checked albums and upload into Google +
